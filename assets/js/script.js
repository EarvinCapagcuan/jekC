...
function myFunction(x) {
    if (navButtonStatus) {
        navUl[0].classList.replace('fadeInDown', 'fadeOut');
        window.setTimeout(function(){
            navUl[0].classList.remove('fadeOut');
            nav[0].classList.replace('show', 'hide');
        }, 500);
        navButtonStatus = false;
    }else{
        nav[0].classList.replace('hide', 'show');
        navUl[0].classList.add('fadeInDown');
...
function loadUrl(x){
    let win = document.getElementsByClassName('wrapper');
    win[0].classList.replace('fadeIn', 'fadeOut');
    setTimeout(function(){
